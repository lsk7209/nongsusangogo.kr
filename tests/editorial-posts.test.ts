...
describe("editorial post package", () => {
  it("contains 23 priority publish-ready Codex-only posts", () => {
    expect(editorialPosts).toHaveLength(23);
    expect(editorialPosts.every((post) => post.codexOnly)).toBe(true);
  });

  it("keeps titles, slugs, and main keywords unique", () => {
    expect(new Set(editorialPosts.map((post) => post.title)).size).toBe(23);
    expect(new Set(editorialPosts.map((post) => post.slug)).size).toBe(23);
